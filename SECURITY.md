...
All cryptographic code lives in a single file: [`packages/crypto/src/crypto.ts`](packages/crypto/src/crypto.ts) (824 lines). Reviewers should start there — it contains every call to the underlying primitives. The desktop app additionally runs Argon2id and XChaCha20-Poly1305 natively in Rust via [`packages/desktop/src-tauri/src/crypto.rs`](packages/desktop/src-tauri/src/crypto.rs); derived keys never cross the JS/Rust boundary on desktop.
